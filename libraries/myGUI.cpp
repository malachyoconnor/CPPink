...
void Gui::UpdatePartOfScreen(Point p1, Point p2) {
   ...
   EPD_7IN5_V2_Display_Part(getPixelCopyForScreen().data(), topLeft.x,
...
BoundingBox Gui::DrawChar(char toDraw, Point bottomLeftBoundary) {
   ...
   if (not SCREEN_BOUNDS.contains(bottomLeftBoundary) or
       not SCREEN_BOUNDS.contains(bottomLeftBoundary +
                                  Point{Font24.Width, 0})) {
      throw std::runtime_error("Trying to draw a character out of bounds!");
   }

   auto [fontTable, fontWidth, fontHeight] = Font24;

   Point topLeftBoundary = {bottomLeftBoundary.x,
                            bottomLeftBoundary.y + fontHeight};
...
BoundingBox Gui::DrawText(string stringToDraw, Point bottomLeftBoundary) {
   ...
   return {{bottomLeftBoundary.x, bottomLeftBoundary.y + Font24.Height},
