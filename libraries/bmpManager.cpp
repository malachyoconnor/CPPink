...
std::unique_ptr<BmpImage> OpenBMP(const filesystem::path& path) {
   ...
   fileStream.read(reinterpret_cast<char*>(&bmpFileHeader),
                   sizeof(BMP_FILE_HEADER));
   fileStream.read(reinterpret_cast<char*>(&bmpInfoHeader),
   ...
   fileStream.read(reinterpret_cast<istream::char_type*>(&colourTableBuffer),
                   8);
   ...
   fileStream.read(reinterpret_cast<char*>(bmpDataVector.data()),
                   imageBytesToRead);
   fileStream.close();

   return make_unique<BmpImage>(
      BmpImage{bmpFileHeader, bmpInfoHeader, bmpDataVector});
}
...
int SaveBMP(const filesystem::path& location, BmpImage& bmpImage) {
   ...
   fileStream.write(reinterpret_cast<char*>(&bmpImage.bmpFileHeader),
                    sizeof(BMP_FILE_HEADER));
   fileStream.write(reinterpret_cast<char*>(&bmpImage.bmpInfoHeader),
                    sizeof(BMP_INFO_HEADER));
   fileStream.write(reinterpret_cast<char*>(&bmpImage.colourTable),
                    sizeof(COLOUR_TABLE));

   fileStream.write(reinterpret_cast<char*>(bmpImage.data.data()),
                    bmpImage.data.size());
   fileStream.close();
...
BmpImage CreateBMP(const PIXEL_ARRAY& pixels) {
   ...
   return BmpImage{bmpFileHeader, bmpInfoHeader, dataVec, COLOUR_TABLE};
}
